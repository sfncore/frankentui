...
//!
//! Top: List of messages (From, Subject, Priority icon, timestamp).
//! Bottom: Selected message body in a Paragraph.
//! Data source: simulated mail messages (deterministic for testing).
//! Supports unread/read styling and mark-read on Enter.
...
/// Priority level for a mail message.
...
/// Deterministic hash for simulated data (same approach as data.rs).
...
/// Generate simulated mail messages (deterministic).
fn generate_messages() -> Vec<MailMessage> {
    ...
    for i in 0..20 {
        ...
        let read = (h >> 48) % 3 == 0; // ~1/3 pre-read
...
pub struct MailInbox {
    ...
    list_scroll: usize,
    /// Scroll offset for the reading pane body.
    body_scroll: usize,
    /// Which panel has focus.
    focus: Panel,
    /// Tick counter for animations.
    tick_count: u64,
    /// Cached layout: message list area.
    layout_list: Cell<Rect>,
    /// Cached layout: reading pane area.
    layout_reading: Cell<Rect>,
    /// Cached layout: status bar area.
    layout_status: Cell<Rect>,
}

impl Default for MailInbox {
    fn default() -> Self {
        Self::new()
    }
}
...
impl MailInbox {
    ...
    fn render_message_list(&self, frame: &mut Frame, area: Rect) {
        ...
        let block = Block::new()
            ...
            .style(theme::panel_border_style(
                self.focus == Panel::MessageList,
                theme::screen_accent::ACTION_TIMELINE, // reuse a blue-ish accent
            ));
        ...
        let list_height = (inner.height as usize).saturating_sub(1); // minus header
        if list_height == 0 {
            return;
        }
        ...
        for (i, msg) in self.messages[scroll..end].iter().enumerate() {
            let row_y = inner.y + 1 + i as u16; // +1 for header
    ...
    fn render_reading_pane(&self, frame: &mut Frame, area: Rect) {
        ...
        let block = Block::new()
            ...
            .style(theme::panel_border_style(
                self.focus == Panel::ReadingPane,
                theme::screen_accent::ACTION_TIMELINE,
            ));
        let inner = block.inner(area);
        block.render(area, frame);
        ...
        // Separator
        let sep = "─".repeat(header_rows[2].width as usize);
...
impl Screen for MailInbox {
    ...
    fn view(&self, frame: &mut Frame, area: Rect) {
        ...
        // Vertical split: message list (top 40%) + reading pane (bottom) + status bar
        ...
        self.layout_status.set(v_chunks[2]);

        chrome::register_pane_hit(frame, v_chunks[0], ScreenId::MailInbox);
        chrome::register_pane_hit(frame, v_chunks[1], ScreenId::MailInbox);
        chrome::register_pane_hit(frame, v_chunks[2], ScreenId::MailInbox);
    ...
    fn keybindings(&self) -> Vec<HelpEntry> {
        vec![
            HelpEntry {
                key: "j/k",
                action: "Navigate messages / scroll body",
            },
            HelpEntry {
                key: "Tab",
                action: "Switch focus (list/reading)",
            },
            HelpEntry {
                key: "Enter",
                action: "Mark as read",
            ...
            HelpEntry {
                key: "g/G",
                action: "First/last message",
            },
            HelpEntry {
                key: "Mouse",
                action: "Click to select/focus",
            },
        ]
    }
...
mod tests {
    ...
    #[test]
    fn mail_inbox_default() {
        let screen = MailInbox::new();
        assert_eq!(screen.title(), "Mail Inbox");
        assert_eq!(screen.tab_label(), "Mail");
    ...
    fn messages_are_deterministic() {
        let a = generate_messages();
        let b = generate_messages();
        assert_eq!(a.len(), b.len());
        for (ma, mb) in a.iter().zip(b.iter()) {
            assert_eq!(ma.from, mb.from);
            assert_eq!(ma.subject, mb.subject);
            assert_eq!(ma.priority, mb.priority);
            assert_eq!(ma.read, mb.read);
        }
    }

    #[test]
    fn mark_read_works() {
        let mut screen = MailInbox::new();
        // Find first unread
        if let Some(idx) = screen.messages.iter().position(|m| !m.read) {
            screen.selected = idx;
            assert!(!screen.messages[idx].read);
            screen.mark_selected_read();
            assert!(screen.messages[idx].read);
        }
    }

    #[test]
    fn toggle_read_works() {
        let mut screen = MailInbox::new();
        let was_read = screen.messages[0].read;
        screen.toggle_selected_read();
        assert_eq!(screen.messages[0].read, !was_read);
        screen.toggle_selected_read();
        assert_eq!(screen.messages[0].read, was_read);
    }
    ...
    fn navigation_bounds() {
        ...
        let last = screen.messages.len() - 1;
        screen.selected = last;
        screen.select_down();
        assert_eq!(screen.selected, last);
    }

    #[test]
    fn unread_count() {
        let screen = MailInbox::new();
        let expected = screen.messages.iter().filter(|m| !m.read).count();
        assert_eq!(screen.unread_count(), expected);
    }
    ...
    fn key_events() {
        ...
        assert_eq!(screen.focus, Panel::MessageList);
        screen.update(&tab);
        assert_eq!(screen.focus, Panel::ReadingPane);
        screen.update(&tab);
        assert_eq!(screen.focus, Panel::MessageList);
    }
