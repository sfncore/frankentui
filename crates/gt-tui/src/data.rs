...
// --- Formula types matching `gt formula list/show --json` ---
...
pub fn fetch_formulas() -> Vec<FormulaItem> {
    if let Some(output) = run_gt(&["formula", "list", "--json"]) {
        let json_start = output.find('[').unwrap_or(0);
        let json = &output[json_start..];
        if let Ok(items) = serde_json::from_str::<Vec<FormulaItem>>(json) {
            return items;
        }
    }
    Vec::new()
...
pub fn fetch_formula_detail(name: &str) -> Option<FormulaDetail> {
    let output = run_gt(&["formula", "show", name, "--json"])?;
    let json_start = output.find('{').unwrap_or(0);
    let json = &output[json_start..];
    serde_json::from_str::<FormulaDetail>(json).ok()
}
...
pub struct FormulaPoller;

impl Subscription<Msg> for FormulaPoller {
    fn id(&self) -> SubId {
        0x464F_524D // "FORM"
    }
    ...
    fn run(&self, sender: mpsc::Sender<Msg>, stop: StopSignal) {
        loop {
            let formulas = fetch_formulas();
            if sender.send(Msg::FormulasRefresh(formulas)).is_err() {
                break;
            }
