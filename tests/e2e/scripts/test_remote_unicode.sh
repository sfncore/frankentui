...
#
# Tests CJK, combining marks, fullwidth, emoji, and box drawing
...
RESULT="$(remote_run_scenario "$SCENARIOS_DIR/unicode_rendering.json" \
    --jsonl "$JSONL_OUT" \
...
if [[ "$OUTCOME" == "pass" ]]; then
    echo "[PASS] Remote unicode rendering"
...
" 2>/dev/null || true
    ...
    echo "[FAIL] Remote unicode rendering: $OUTCOME"
    exit 1
fi
